# MoreVanillaMod✨
Mindustry mod by adding functional buildings and units to help in campaign mode (only serpulo).

even tanks!

# Information🗒️
I will give you two interesting information from this mod.

1. Support unit, Node. Nodes will help you build buildings and mine ore, if you command them.

2. Resource Extractor: There are two versions of the Resource Extractor: the original and the second. As the name suggests, this building extracts resources using water and power. the output is random, usually copper, tin, graphite, etc.

# Change Logs📝

#1.6
-First global release
-added the 2nd version of the serpulo tank
-memperbaiki bug sprite dari fixed sprite bug from previous tank

#1.5
-added new tank for serpulo

#1.4
-added an advanced version of Resource Extractor, Extreme Resource Extractor

#1.3
-improve compatibility on Resource Extractor 

#1.2
-adding resource extractor

#1.1
-fixing drone and other block

#1.0
-adding copper delivery drones
-added unloading point and copper drone unloader point
